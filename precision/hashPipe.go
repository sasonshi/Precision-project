...
func runHashPipe(flowFrequency []int) []int {
...
func (pip *pipeline) processHashPipe(packet Packet) {
	var ckey int
	var cval int
	firstStage := pip.stages[0]
	l := firstStage.hashFunc(packet.flowID)
	if firstStage.keys[l] == packet.flowID {
		firstStage.values[l] += 1
		return
	} else if firstStage.keys[l] == 0 { //empty slot
		firstStage.keys[l] = packet.flowID
		firstStage.values[l] = 1
		return
	} else {
		ckey, cval = firstStage.keys[l], firstStage.values[l]
		firstStage.keys[l] = packet.flowID
		firstStage.values[l] = 1
	}
	for i := 1; i < d; i++ {
		currStage := pip.stages[i]
		l = currStage.hashFunc(ckey)
		if currStage.keys[l] == ckey {
			currStage.values[l] += cval
			return
		} else if currStage.keys[l] == 0 { //empty slot
			currStage.keys[l] = ckey
			currStage.values[l] = cval
			return
		} else if currStage.values[l] < cval { //swap
			keyTemp, valTemp := ckey, cval
			ckey, cval = currStage.keys[l], currStage.values[l]
			currStage.keys[l], currStage.values[l] = keyTemp, valTemp
		}
	}
...
func topKHashPipe() {
	...
	for i, initVal := range initValues {
		...
		for i, num := range numOfCounters {
			...
			initValue = initVal
			hashPipeFreq := runHashPipe(flowFrequency)
			y[i] = topKProblem(hashPipeFreq, flowFrequency, k)
		}
...
func compareInitValueHashPipe() {
	...
	// Set plot title and labels
	p.Title.Text = "Init Value Hash Pipe Comparison"
	...
	// Save the plot to a file
	err := p.Save(4*vg.Inch, 4*vg.Inch, "Init Value Hash Pipe Comparison.png")
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println("Init Value Hash Pipe Comparison.png")
}
